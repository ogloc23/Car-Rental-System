        ...
        verifyPayment: async (_, { reference }, context) => {
            try {
                ...
                if (error instanceof GraphQLError)
                    throw error;
                throw new GraphQLError(error instanceof Error ? error.message : "An error occurred during payment verification.", { extensions: { code: "INTERNAL_SERVER_ERROR" } });
            }
        },
    },
    Mutation: {
        initializePayment: async (_, { email, amount }, context) => {
            try {
                if (!context.user) {
                ...
                if (error instanceof GraphQLError)
                    throw error;
                throw new GraphQLError(error instanceof Error ? error.message : "An error occurred while initializing payment.", { extensions: { code: "INTERNAL_SERVER_ERROR" } });
            }
        },
    },
};
